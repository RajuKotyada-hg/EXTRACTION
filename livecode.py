...
class DataFramePreparation:
    def __init__(self, config: dict):
        ...
        self.net_Amount_label = config.get('net_Amount_label', False)
    ...
    def process_dataframe(self, df:pd.DataFrame, config : dict):
        ...
        if 'triggers' in config:
            ...
            for item in filtered_data:
                ...
                # filtered_dataframe = [row for row in dataframe if row.get('lineitem_desc', '') != '' or row.get('net_Amount', '') != '']
                filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '']
                # filtered_dataframe = [row for row in dataframe if row['net_Amount'] != ' ' or row['stat'] != ' '] 
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '']
                # for row in filtered_dataframe['dataframe']:
                #     for key in list(row.keys()):
                #         if pd.isna(row[key]):
                #             del row[key]
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '' or row['net_Amount']>0]
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '']
                # filtered_dataframe = [row for row in dataframe if row['net_Amount'] != '']
                # filtered_dataframe = dataframe[(dataframe['lineitem_desc'] != '') | (dataframe['net_Amount'] != '')]
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != ' ']
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != ' ' or row['lineitem_desc'] != 'nan' or 'net_Amount' in row]
                # filtered_dataframe = [row for row in dataframe if (row.get('lineitem_desc', '').strip() not in ['', 'nan']) or ('net_Amount' in row)]
            ...
            for item in filtered_data:
                ...
                #             del row[key]
                # filtered_dataframe = [row for row in dataframe if row.get('lineitem_desc', '') != '' or row.get('net_Amount', '') != '']
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '' or row['net_Amount'] != '' or row['lineitem_desc'].notna()]
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '']
                # filtered_dataframe = [row for row in dataframe if row['net_Amount'] != '']
                # filtered_dataframe = dataframe[(dataframe['lineitem_desc'] != '') | (dataframe['net_Amount'] != '')]
                # filtered_dataframe = [row for row in dataframe if row['lineitem_desc'] != '' or {'net_Amount' in row}]
                # filtered_dataframe = [row for row in dataframe if (row.get('lineitem_desc', '').strip() not in ['', 'nan']) or ('net_Amount' in row)]
                item['dataframe'] = filtered_dataframe
                final_data.append(item)
            # print(final_data)
    ...
    def split_stat_amount(self,  df : pd.DataFrame, config:dict | None = None):
        ...
        try:
            ...
            df.loc[mask, 'net_Amount'] = df.loc[mask, config['value_col']]
    ...
    def add_netAmount_col(self,  df : pd.DataFrame):
        try:
            if(len(self.net_Amount_label) == 1):
                df['net_Amount'] = df[self.net_Amount_label[0]].apply(self.format_amount)  

            elif(len(self.net_Amount_label) > 1):
                df['net_Amount'] = df[self.net_Amount_label[0]].apply(self.format_amount) - sum(df[x].apply(self.format_amount) for x in self.net_Amount_label[1:])

        except Exception as e:
            print('🐞🐞 Error while add_netAmount_col: ', e)
        return df

    def Apply_PMS_Rules(self,df:pd.DataFrame, ignore_words):
        output = {'dataframe':df}
        try:
...
#                 df.columns = range(len(df.columns))
                ...
                if self.net_Amount_label !=False:
            ...
            for idx,tr in enumerate(config):
                ...
                if idx+1 <= len(config) and 'end' in tr:
                    ...
                    if start_index != None and end_index != None:
                        res= self.Apply_PMS_Rules(df[ start_index:end_index], tr['ignore_words'] )
                        dfs_ids.append(res)
                    # else:
...
input_pdf = "C:\\Users\\raju.kotyada\\Downloads\\ALLPMS\\MAPPED\\18. Roomkey\\Account Receivable Reconciliation Report.pdf"
